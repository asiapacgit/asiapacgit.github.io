...
$(function () {
    ...
    const reheightHome = () => {
        document.querySelector('.services-home .owl-carousel').querySelectorAll('.item').forEach(item => {
            item.style.height = 'auto'
        })
        let nodeArray = Array.from(document.querySelector('.services-home .owl-carousel').querySelectorAll('.item'))
        let highestHeight = Math.max(...nodeArray.map(node => node.clientHeight));
    ...
    const reheight = () => {
        document.querySelector('.testimonials .owl-carousel').querySelectorAll('.item').forEach(item => {
            item.style.height = 'auto'
        })
        let nodeArray = Array.from(document.querySelector('.testimonials .owl-carousel').querySelectorAll('.item'))
        let highestHeight = Math.max(...nodeArray.map(node => node.clientHeight));
        console.log(highestHeight)
        document.querySelector('.testimonials .owl-carousel').querySelectorAll('.item').forEach(item => {
            item.style.height = highestHeight + 'px'
...
(($) => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const rows = document.querySelector('.portfolio.services.page-service')
        if (!rows) return;
        console.log(rows)
        const columns = rows.querySelectorAll(".col-lg-4, .col-md-6, .col-12");
        ...
        function setEqualHeights() {
            ...
            columns.forEach(column => {
                const item = column.querySelector(".item");
    
                // Reset the height to auto for recalculating
                item.style.height = "auto";
    
                // Get the top position of the column
                const topPosition = column.getBoundingClientRect().top;
    
                // If it's a new row
                if (currentRowTop === null || topPosition > currentRowTop) {
                    // Adjust heights for the previous row group
                    if (rowTracker.length > 0) {
                        adjustHeights(rowTracker);
                    }
    
                    // Start a new row group
                    currentRowTop = topPosition;
                    rowTracker = [];
                }
    
                // Add the column's .item to the current row group
                rowTracker.push(item);
            });
    
            // Adjust heights for the last row group
            if (rowTracker.length > 0) {
                adjustHeights(rowTracker);
            }
        }
    
        function adjustHeights(items) {
            let maxHeight = 0;
...
(($) => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const rows = document.querySelector('.same-height')
        if (!rows) return;
        console.log(rows)
        const columns = rows.querySelectorAll(".item");
...
(($) => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const rows = document.querySelector('.contact-box')
        if (!rows) return;
        console.log(rows)
        const columns = rows.querySelectorAll(".col-lg-4, .col-md-6");
...
(() => {
    document.querySelectorAll('.custom-modal-trigger').forEach(el => {
        el.addEventListener('click', function () {
            ...
            jQuery(modalElement).find('.target-name').text(el.getAttribute('data-name'))
            jQuery(modalElement).find('.target-image').attr('src', el.getAttribute('data-image'))
            jQuery(modalElement).find('.target-title').text(el.getAttribute('data-title'))
            // modalElement.querySelector('.target-name')
        
            // Show the modal
            modal.show();
        
            // Optional: Listen for modal events
            modalElement.addEventListener('shown.bs.modal', () => {
              console.log('Full-Screen Modal is shown');
            });
        
            modalElement.addEventListener('hidden.bs.modal', () => {
              console.log('Full-Screen Modal is hidden');
